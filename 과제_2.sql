...
select max(salary) as '최고임금', min(salary) '최저임금', max(salary)-min(salary) '최고임금 – 최저임금'
from salaries;
...
select date_format(now(),'%Y')-substring(min(birth_date),1,4), 
	   date_format(now(),'%Y')-substring(max(birth_date),1,4)
       from employees;
